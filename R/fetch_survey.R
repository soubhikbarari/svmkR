      ...
      if ("answers" %in% names(question)) {
        ...
        if ("choices" %in% names(question$answers)) {
          for (choice in question$answers$choices) {
            answers[[choice$id]] <- trimws(
              choice$text
            )
            levels[[question$id]] <- c(levels[[question$id]], answers[[choice$id]])
          }
        }
        if ("other" %in% names(question$answers)) {
          answers[[question$answers$other$id]] <- trimws(
