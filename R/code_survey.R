...
#' @export 
code_std_demo_vars <- function(data, verbose = TRUE) {
  data <- data %>%
    code_party_vars(., verbose) %>%
    code_ideo_vars(., verbose) %>%
    code_age_vars(., verbose) %>%
    code_race_vars(., verbose) %>%
    code_sex_vars(., verbose) %>%
    code_educ_vars(., verbose) %>%
    code_income_vars(., verbose) %>%
    code_geo_vars(., verbose)    
}
...
#' }
code_party_vars <- function(data, verbose = TRUE) {
  ...
  if (is_valid(party_id)) {
    if (is_valid(party_lean)) {
      ...
      data <- data %>%
        ...
        mutate(party5 = magrittr::set_attr(party3, "label", "Party ID (3-point)"))
...
#' }
code_ideo_vars <- function(data, verbose = TRUE) {
...
#' }
code_age_vars <- function(data, verbose = TRUE) {
...
#' }
code_race_vars <- function(data, verbose = TRUE) {
  ...
  if (is_valid(race) & is_valid(hispanic)) {
    data <- data %>%
      ...
      dplyr::mutate(race2 = forcats::fct_collapse(race5,
                                                  "White" = "White",
                                                  "Non-White" = c("Black", "Hispanic", "Asian", "Other"))) %>%
      dplyr::mutate(race2 = forcats::fct_relevel(race2, "White", "Non-White")) %>%
      dplyr::select(-(race_1:race_8), -sum_race) %>%
      dplyr::mutate(race5 = magrittr::set_attr(age, "label", "Race (5-point)"),
                    race4 = magrittr::set_attr(age3, "label", "Race (4-point)"),
                    nonwhite = magrittr::set_attr(age6, "label", "Race (White/Non-white)"))
...
#' }
code_sex_vars <- function(data, verbose = TRUE) {
...
#' }
code_educ_vars <- function(data, verbose = TRUE) {
...
#' }
code_income_vars <- function(data, verbose = TRUE) {
  ...
  if (is_valid(inc)) {  
    data <- data %>%
      ...
      dplyr::mutate(income3 = magrittr::set_attr(income3, "label", "Household income (3-point)"))
...
#' }
code_geo_vars <- function(data, verbose = TRUE) {
  ...
  if (is_valid(zi)) {  
    ...
    zips_xw <- suppressMessages(readr::read_csv(zips_fpath)) %>%
      dplyr::select(zip=ZIP, state_fips=STATE, state_abb=ST) %>%
